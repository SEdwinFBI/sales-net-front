...
export function getPasskeyErrorMessage(error: unknown): string {
  if (error instanceof DOMException) {
    ...
    if (error.name === 'InvalidStateError') return 'Este acceso ya está registrado. Usa otro acceso o ingresa con el existente.'
    if (error.name === 'SecurityError') return 'No se pudo validar el dominio de la aplicación. Contacta al administrador.'
    if (error.name === 'NotSupportedError') return 'Tu dispositivo no admite esta configuración de acceso. Usa el acceso con contraseña.'
  }
