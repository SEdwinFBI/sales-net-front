...
export const getClientes = async (page = 1, pageSize = 10, search = '', activo?: boolean, permitirCredito?: boolean): Promise<{ count: number; results: Cliente[] }> => {
  const { data } = await api.get<ApiResponse<Cliente[] | { count: number; results: Cliente[] }>>('/admin/clientes/', {
    params: { page, page_size: pageSize, ...(search ? { search } : {}), ...(activo !== undefined ? { activo } : {}), ...(permitirCredito !== undefined ? { permitir_credito: permitirCredito } : {}) },
  })
